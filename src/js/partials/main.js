...
$(function () {
  var LearnWords = function () {
    function LearnWords() {
      _classCallCheck(this, LearnWords);
    ...
    _createClass(LearnWords, [{
          ...
          case 'no':
            {
              this.wrong++;
              if (this.left > 1) {
                var tmp = this.workmas.splice(this.left - 1, 1);
          ...
          case 'progressbar':
            {
              this.playedRoundsRU = 0;
              this.playedRoundsEN = 0;
              var mas = event.currentTarget.innerText.split('\n');
      ...
      value: function DrawInterface() {
        var isAnswer = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : false;
      ...
      value: function DrawProgressBar() {
        $('.grid_item').remove();
        var countOfBlocks = Math.ceil(this.len / this.split);
        var tmpFrom = 1;
        var tmpTo = this.split;
        var $html = void 0;
      ...
      value: function ShuffleData() {
        ...
        var calculate = this.to - (this.from - 1);
